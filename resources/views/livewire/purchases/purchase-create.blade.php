                ...
                @foreach ($items as $index => $item)
                    <div wire:key="purchase-item-{{ $index }}" class="grid grid-cols-1 gap-3 rounded-xl border border-black/10 bg-white/50 p-4 sm:grid-cols-6">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0.01" wire:model.live="items.{{ $index }}.cost_price" class="mt-1 min-h-[44px] w-full rounded-xl border border-black/10 bg-white/70 px-3 py-2 text-sm font-medium text-[var(--text-primary)]">
                            <x-input-error :messages="$errors->get('items.'.$index.'.cost_price')" class="mt-1" />
                        </div>
                        <div class="flex items-end gap-2">
                            <div class="flex-1">
                                <x-input-label :value="__('purchases.quantity')" />
                                <input type="number" step="1" min="1" wire:model.live="items.{{ $index }}.quantity" class="mt-1 min-h-[44px] w-full rounded-xl border border-black/10 bg-white/70 px-3 py-2 text-sm font-medium text-[var(--text-primary)]">
