<div>
    <x-page-header>
        <div class="flex flex-wrap items-center justify-between gap-3">
            <h2 class="text-2xl font-bold text-[var(--text-primary)]">{{ __('expenses.categories_title') }}</h2>
            ...
            <table class="w-full min-w-[480px] text-start">
                <thead>
                    <tr class="border-b border-black/10 text-sm font-semibold text-[var(--text-secondary)]">
                        <th class="px-3 py-2 text-start">{{ __('expenses.category_name') }}</th>
                        <th class="px-3 py-2 text-end">{{ __('expenses.expense_count') }}</th>
                        <th class="px-3 py-2 text-center">{{ __('expenses.status') }}</th>
                ...
                <tbody>
                    @forelse ($categories as $category)
                        <tr class="border-b border-black/5 text-base font-medium text-[var(--text-primary)]">
                            ...
                            <td class="px-3 py-3 text-end">{{ $category->expenses_count }}</td>
                            <td class="px-3 py-3 text-center">
                                <span class="rounded-full px-3 py-1 text-xs font-bold {{ $category->is_active ? 'bg-[var(--color-success)]/10 text-[var(--color-success)]' : 'bg-[var(--color-danger)]/10 text-[var(--color-danger)]' }}">
                                    {{ $category->is_active ? __('expenses.active') : __('expenses.inactive') }}
                                </span>
                            </td>
                            <td class="px-3 py-3 text-end">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button type="button" wire:click="toggleActive({{ $category->id }})" class="min-h-[44px] rounded-lg px-3 py-2 text-sm font-semibold text-[var(--color-danger)] hover:bg-black/5">
                                        {{ $category->is_active ? __('expenses.deactivate') : __('expenses.activate') }}
                                    </button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-3 py-6 text-center text-[var(--text-secondary)]">{{ __('expenses.no_categories') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <x-glass-modal show="showModal">
        <form wire:submit="save" class="space-y-4">
            <h3 class="text-xl font-bold text-[var(--text-primary)]">
                {{ $editingCategoryId ? __('expenses.edit_category') : __('expenses.add_category') }}
            </h3>

            <div>
                <x-input-label for="name" :value="__('expenses.category_name')" />
